...
assert(index.includes('direct-static-v11'), 'index.html contains the direct-static-v11 build marker');
assert(!index.includes('safetrack-v10'), 'index.html contains no v10 overlay asset');

const forbidden = [
  'DecompressionStream','catalog50-v1/part','atob(','pako.',
  'Failed to Decode Data','build-direct-static.py','safetrack-v10.js','safetrack-v10.css'
];
const publicSource = Object.values(sources).join('\n');
for (const token of forbidden) {
  assert(!publicSource.includes(token), `forbidden legacy or overlay token is absent: ${token}`);
}
...
assert(groups.includes('aria-expanded="false"') && groups.includes('class="st-category-body"') && groups.includes('hidden'),
  'all training categories are rendered collapsed by default');
assert(groups.includes("const job = entry.employee[3]") && groups.includes('class="st-job-group"'),
  'employee list groups only actual assigned Tätigkeit values');
assert(groups.includes('data-st-action="select-critical"') && groups.includes('data-st-action="select-soon"'),
  'critical and 6–30 day bulk selections exist');
assert(groups.includes('data-st-print-mode="confirmation"') && groups.includes('data-st-print-mode="full"'),
  'both print modes exist');
assert(groups.includes('Durchgeführt am') &&
  groups.includes('Personalnummer der unterweisenden / beaufsichtigenden Person') &&
  groups.includes('Name der unterweisenden / beaufsichtigenden Person'),
  'confirmation contains completion date and supervisor identity fields');
assert(groups.includes('Unterschrift Mitarbeitende:r') &&
  groups.includes('Unterschrift unterweisende / beaufsichtigende Person'),
  'confirmation contains separate employee and supervisor signatures');

const header = groups.slice(groups.indexOf('function printHeader'), groups.indexOf('function confirmationFields'));
assert(!header.includes('Fällig') && !header.includes('Status'),
  'print header omits due date and status');
assert(groups.includes("sheet.dataset.printVersion = 'v11'"), 'unified print output identifies v11');
assert(printCss.includes('.st-confirmation-page{display:block') &&
  printCss.includes('break-inside:avoid-page'),
  'confirmation uses compact normal-flow pagination');
for (const fixedToken of ['height:245mm','min-height:245mm','max-height:245mm','grid-template-rows:auto 1fr auto']) {
  assert(!printCss.includes(fixedToken), `print CSS does not contain fixed spacer token: ${fixedToken}`);
}
assert(printCss.includes('@page{size:A4;margin:10mm}'), 'print CSS targets compact A4 output');
assert(printCss.includes('.st-source-table{display:none!important}'), 'flat employee table is hidden');
