...
52. Group Schulungsbestätigung output must generate one independent participant-list document per selected training. Each participant must occupy exactly one row with the four columns `Mitarbeitende Person`, `PNr.`, `Tätigkeit` and `Unterschrift`. The completion date belongs in the document header, not in a participant-row column. The document must also contain training ID, training version, participant count and trainer/supervisor name, personnel number and signature field.
...
57. A group Schulungsbestätigung regression test must verify exactly four participant columns, exactly one table row per participant, and a long mobile participant list before release.
